...
if  p'*L*p/m<0.001%0.31-0.01*n_thr
    t = 0;
end
...
det(inv(eye(m) + lambda_2*P)-(eye(m)+lambda_1*L));
%%
iter = 5000; %500; 
loss = zeros(iter,1);
...
for ii = 1:iter
    ...
    loss2(ii) = norm(y - n_est.*p_est).^2 + lambda_1*p'*L*p ...
        + lambda_2*norm(n-X*my_beta).^2;
    
    loss(ii) = norm(logy - (u+v)).^2 + lambda_1*v'*L*v ...
        + lambda_2*u'*P*u;
    
end
err1(tt)=sum(abs(n_est-n))/sum(n);
err2(tt)=sum(abs(p_est-p))/sum(p);
%if sum(abs(n_est-n))/sum(n)>0.5
%    break
%end
end
er_n(n_thr) = sum(err1)/epoch;
er_p(n_thr) = sum(err2)/epoch;
end

plot(er_n)
ylabel("relative l1 error of n")
xlabel("Upper bound of p^TLp")
%%
figure(1);clf; 
subplot(121);
plot(loss,'-o'); 
